"""Market entity metadata derived from market scenes."""

from __future__ import annotations

from dataclasses import dataclass, field
from typing import Any


@dataclass(frozen=True)
class MarketEntity:
    """A structural market entity without interpretation or decision-making."""

    id: str
    source_market_scene_id: str
    source_market_element_id: str
    source_semantic_element_id: str
    source_object_id: str
    stable_key: str
    entity_type: str
    region_id: str
    region_name: str
    text: str
    labels: tuple[str, ...]
    x: int
    y: int
    width: int
    height: int
    confidence: float
    created_at: str
    metadata: dict[str, Any] = field(default_factory=dict)

    def to_dict(self) -> dict[str, object]:
        """Return a serializable representation."""
        return {
            "id": self.id,
            "source_market_scene_id": self.source_market_scene_id,
            "source_market_element_id": self.source_market_element_id,
            "source_semantic_element_id": self.source_semantic_element_id,
            "source_object_id": self.source_object_id,
            "stable_key": self.stable_key,
            "entity_type": self.entity_type,
            "region_id": self.region_id,
            "region_name": self.region_name,
            "text": self.text,
            "labels": list(self.labels),
            "position": {
                "x": self.x,
                "y": self.y,
                "width": self.width,
                "height": self.height,
            },
            "confidence": self.confidence,
            "created_at": self.created_at,
            "metadata": dict(self.metadata),
        }


@dataclass(frozen=True)
class MarketEntities:
    """Collection of market entities derived from one market scene."""

    source_market_scene_id: str
    source_visual_scene_id: str
    source_frame: str
    created_at: str
    entities: tuple[MarketEntity, ...]

    @property
    def count(self) -> int:
        """Return the number of market entities."""
        return len(self.entities)

    @property
    def region_count(self) -> int:
        """Return the number of regions represented by market entities."""
        return len({entity.region_id for entity in self.entities})

    @property
    def element_count(self) -> int:
        """Return the number of source market elements represented."""
        return self.count

    def to_dict(self) -> dict[str, object]:
        """Return a serializable representation."""
        return {
            "source_market_scene_id": self.source_market_scene_id,
            "source_visual_scene_id": self.source_visual_scene_id,
            "source_frame": self.source_frame,
            "created_at": self.created_at,
            "count": self.count,
            "region_count": self.region_count,
            "element_count": self.element_count,
            "entities": [entity.to_dict() for entity in self.entities],
        }
