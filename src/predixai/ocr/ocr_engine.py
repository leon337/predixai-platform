...
_CACHE_SCHEMA_VERSION = "PTP-GOV.4.6C.1B.3"


class OCREngine:
    """Execute OCR with strict configuration and cache boundaries."""

    def __init__(self, config: dict[str, Any] | None = None) -> None:
        self.config = config or {}
        raw_min_confidence = self.config.get("min_confidence", 0.0)
        if isinstance(raw_min_confidence, bool):
            raise ValueError(
                "OCR minimum confidence must be numeric, not boolean"
            )
        min_confidence = float(raw_min_confidence)
        if (
            not math.isfinite(min_confidence)
            or not 0.0 <= min_confidence <= 100.0
        ):
            raise ValueError(
                "OCR minimum confidence must be between 0 and 100"
            )
        ...
        raw_psm = self.config.get("psm", 6)
        if isinstance(raw_psm, bool) or not isinstance(raw_psm, int):
            raise ValueError("OCR PSM must be an integer")
        raw_timeout = self.config.get("timeout_seconds", 20)
        if isinstance(raw_timeout, bool):
            raise ValueError("OCR timeout must be numeric, not boolean")
    ...
    def prepare_pipeline(self, image_path: str | Path) -> OCRResult:
        ...
        provider = self.selector.select(
            str(self.config.get("provider", "mock"))
        )
        provider_status = provider.load()
        extraction_requested = bool(
            self.config.get("text_extraction_enabled", False)
        )
        ...
        privacy_sensitive = bool(
            self.config.get("privacy_sensitive", False)
        )
        ...
        if not provider_status.ready:
            execution = OCRProviderExecution(
                ...
                0.0,
                "",
                "OCR provider is not ready.",
            )
            benchmark_result = self.benchmark.disabled_result(
                execution,
                False,
            )
        ...
        checked = self.result_validator.validate(
            provider_status,
            execution,
            str(self.config.get("language", "")),
            str(self.config.get("fallback_language", "")),
        )
        pipeline_ready = (
        ...
        result = OCRResult(
            ...
            text_extracted=(
                execution.text_extracted and text_extraction_enabled
            ),
            ...
            provider_language_available=(
                provider_status.language_available
            ),
        )
        ...
        if (
            effective_cache
            and checked.valid
            and result.status == "OCR_COMPLETED"
    ...
    def _load_cached_result(
        ...
        benchmark = self.benchmark.cache_hit_result(
